...
export const Carousel = ({scrollStep, spacing, title, fade, tooltipProps, buttonProps, children, ...props}: CarouselProps) => {
  ...
  return (
    <>
      <Container>
        <Flex justifyContent={isTablet ? 'space-between' : 'center'} zIndex={20} mb={12} px={8}>
          <Heading fontSize={20} fontWeight="400" color="oilblue.500">{title}</Heading>
          {isTablet && (
            <Box>
              <Tooltip label={buttonProps ? buttonProps.labelLeft : "Voltar"} placement="top" {...tooltipProps}>
                <IconButton aria-label="Scroll Back" mr={2} icon={<IoIosArrowBack/>} onClick={handleScrollBack}/>
              </Tooltip>
              <Tooltip label={buttonProps ? buttonProps.labelRight : "Próximo"} placement="top" {...tooltipProps}>
                <IconButton aria-label="Scroll Forward" icon={<IoIosArrowForward/>} onClick={handleScrollNext}/>
              </Tooltip>
            </Box>
          )}
        </Flex>
